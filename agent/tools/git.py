...
GIT_HELP = {
    "pr": "pr [--action=create|list|merge] [--branch=NAME] [--title=TEXT] [--state=open|merged]",
    "issue": "issue [--action=list|create] [--title=TEXT] [--body=TEXT] [--state=open|closed]",
    "repo": "repo [--action=list|create|delete] [--name=NAME]",
}
...
class GitTool(NamespaceTool):
    def __init__(self) -> None:
        self._prs = list(MOCK_PRS)
        self._issues = list(MOCK_ISSUES)
        self._repos = list(MOCK_REPOS)

    def _get_help(self, command: str) -> str:
        if command in GIT_HELP:
            return GIT_HELP[command]
        lines = ["Available commands:"] + [f"  {v}" for v in GIT_HELP.values()]
        return "\n".join(lines)
    ...
    def _handle_pr(self, args: dict) -> str:
    ...
    def _handle_issue(self, args: dict) -> str:
        action = args.get("action", "list")
        if action == "list":
            state = args.get("state")
        ...
        if action == "create":
            ...
            self._issues.append({"id": new_id, "title": title, "body": body, "state": "open"})
            return f"Issue {new_id} created: '{title}'."
        return f"Unknown action '{action}'."

    def _handle_repo(self, args: dict) -> str:
        action = args.get("action", "list")
        if action == "list":
        ...
        if action == "create":
            ...
            self._repos.append({"name": name, "description": desc})
